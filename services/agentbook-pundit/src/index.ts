...
async function doComment() {
  ...
  try {
    ...
    if (active.length === 0) {
      log('ℹ️ No active markets for comments');
      return;
    }
    ...
    if (success) {
      ...
      log('⚠️ Comment submission failed (check signature/auth headers)');
    }
  } catch (err) {
...
async function main() {
  log('🤖 AgentBook Pundit starting (guardrails enabled)...');
  log(`Wallet: ${config.walletAddress}`);
  log(`API: ${config.apiUrl}`);
  log(`Affiliate code configured: ${config.affiliateCode ? 'yes' : 'no'}`);
  log(`LLM for closed-market recaps: ${process.env.GOOGLE_API_KEY ? 'Gemini 2.0 Flash' : 'disabled'}`);
