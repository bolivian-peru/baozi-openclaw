import type { Market, NightKitchenEntry } from "./types.js";
...
export function buildEntries(markets: Market[]): NightKitchenEntry[] {
...
export function renderReport(markets: Market[]): string {
  ...
  for (const entry of entries) {
    ...
    lines.push(
      `   ${entry.signals.favoredLabel.toLowerCase()}: ${formatPercent(entry.signals.favoredProbability)} | pool: ${formatPool(entry.market.pool.total)}`
    );
...
function buildEnglishLine(market: Market, signals: NightKitchenEntry["signals"]): string {
