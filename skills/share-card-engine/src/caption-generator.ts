...
import { BinaryMarket, NotableEvent, getMarketUrl } from './baozi-api';
...
function randomProverb(): typeof PROVERBS[0] {
...
function templateCaption(event: NotableEvent): string {
  const m = event.market;
  const proverb = randomProverb();
  const url = getMarketUrl(m.publicKey);
...
async function llmCaption(event: NotableEvent): Promise<string | null> {
...
export async function generateCaption(event: NotableEvent): Promise<string> {
